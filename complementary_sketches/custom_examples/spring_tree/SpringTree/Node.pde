...
class Node extends PVector {
  ...
  float noiseX = random(500);
  float noiseY = random(500);
  PVector noiseV = new PVector(noiseX,noiseY);
  float step = 0.005;
  ...
  void update(boolean theLockX, boolean theLockY, boolean theLockZ) { 
    ...
    if (!theLockX) x += velocity.x+map(noise(noiseX,10,20),0,1,-1,1);
    if (!theLockY) y += velocity.y+map(noise(noiseY,50,5),0,1,-1,1);
